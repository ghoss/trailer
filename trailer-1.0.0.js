...
RR_Diagram.prototype.dummy_callback = function(data) {
	...
	$(data.target).append(header, data.diagram, footer);
}
...
RR_Diagram.prototype.compose = function(target, grammar, s_header, s_footer) {
...
EBNF.prototype.p_rule = function(grammar) {
	...
	do {
		if (res = this.p_string(grammar)) {
			...
			grammar = $.trim(res.trailer);
		
			if (grammar.substr(0, 1) == ")") {
				// Rule in grouping parentheses
				grammar = $.trim(grammar.slice(1));
				var g0 = "(";
...
EBNF.prototype.display = function(grammar, target) {
